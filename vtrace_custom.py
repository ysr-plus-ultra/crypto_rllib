...
class VTraceCustomLoss:
    def __init__(
        ...
        self.value_targets = normalized_G_t_vtrace
        #
        ...
        # # The baseline loss.
        delta = (normalized_values - self.value_targets.detach()) * valid_mask
...
def build_vtrace_loss(policy, model, dist_class, train_batch):
    model.update_popart(policy.cur_lr)
    ...
    model.tower_stats['final_loss'] = loss.final_loss
...
def stats(policy: Policy, train_batch: SampleBatch) -> Dict[str, Any]:
    ...
    return {
        ...
        "final_loss": torch.mean(torch.stack(policy.get_tower_stats("final_loss"))),
        ...
        "beta": policy.model.beta,
        # "gamma":  1 + (policy.model.alpha * torch.exp(policy.model.beta) - 1) ** 2,
        "mu": policy.model.mu,
        "nu": policy.model.nu,
        "sigma": policy.model.sigma,
    }
...
VTraceCustomPolicy = build_policy_class(
    name="VTraceCustomPolicy",
    framework="torch",
