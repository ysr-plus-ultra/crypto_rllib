...
import numpy as np
import ray
from grokfast_pytorch.grokfast import GrokFastCustom
...
class VTraceOptimizer:
    ...
    def optimizer(
        ...
        #
        # )
        return GrokFastCustom(params = self.model.parameters(),
                             lr = self.cur_lr,
                             beta = 0.99,
                             weight_decay=self.config["decay"],
                             eps=self.config["epsilon"],
...
class ImpalaTorchPolicyCustom(
    ...
    def stats_fn(self, train_batch: SampleBatch) -> Dict[str, TensorType]:
        prev_a = train_batch[SampleBatch.ACTIONS].float()
        result_dict = {
            "cur_lr": self.cur_lr,
        ...
        result_dict["popart/1mu"] = self.model.mu
        result_dict["popart/2sigma"] = self.model.sigma
        result_dict["popart/3skewness"] = self.model.skewness
        result_dict["popart/4kurtosis"] = self.model.kurtosis

        # result_dict["custom/1alpha"] = self.model.var_alpha
        # result_dict["custom/2beta"] = self.model.var_beta
